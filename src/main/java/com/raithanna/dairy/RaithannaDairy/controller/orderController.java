...
public class orderController {
    ...
    public String createSaleOrder(@RequestParam String orderNo) {
        ...
        List<dailySales> orderProducts = (List<dailySales>) dailySalesRepository.findByOrderNo(String.valueOf(Integer.parseInt(orderNo)));
        while (true) {
            if (orderProducts.isEmpty()) {
                orderProducts = (List<dailySales>) dailySalesRepository.findByOrderNo(String.valueOf(Integer.parseInt(orderNo)));
            } else {
                break;
            }
        }
        ...
        saleorder.setOrderNo(Integer.valueOf(String.valueOf(orderNo)));
    ...
    public String getOrder(@RequestParam(name = "orderNo", defaultValue = "1") Integer orderNo, Model model) {
        List<dailySales> orderProducts = (List<dailySales>) dailySalesRepository.findByOrderNo(orderNo.toString());
        saleOrder sale_order = saleOrderRepository.findByOrderNo(String.valueOf(orderNo));
        DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
        String formattedDate = sale_order.getDate().format(String.valueOf(dateTimeFormatter));
    ...
    public String viewOrderForm(Model model, @PathVariable(name = "orderNo") Integer orderNo, HttpSession session) {
        if (session.getAttribute("loggedIn").equals("yes")) {
            ...
            List<dailySales> orderedProducts = (List<dailySales>) dailySalesRepository.findByOrderNo(String.valueOf(orderNo));
    ...
    public ModelAndView createOrderNew(Model model, @RequestBody List<dailySales> orderList, HttpSession session) {
        if (session.getAttribute("loggedIn").equals("yes")) {
            ...
            so.setCustCode(orderList.get(0).getCustCode());
            so.setOrderNo(Integer.valueOf(String.valueOf(Integer.valueOf(String.valueOf(orderList.get(0).getOrderNo())))));
            Double d = totalAmount - totalNetAmount;
            so.setDisc(d);
            so.setAmount(totalAmount);
            so.setNetAmount(totalNetAmount);
            so.setDate(String.valueOf(LocalDate.parse((orderList.get(0).getDate()))));
